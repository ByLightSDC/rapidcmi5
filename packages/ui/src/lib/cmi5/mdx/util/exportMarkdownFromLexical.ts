 ...
 */
export interface LexicalExportVisitor<
...
//  * @group Markdown Processing
//  */
type LexicalVisitor = LexicalExportVisitor<LexicalNode, Mdast.RootContent>;
...
export function exportLexicalTreeToMdast({
...
}: ExportLexicalTreeOptions): Mdast.RootContent {
  let unistRoot: Mdast.RootContent | null = null;
  ...
  function registerReferredComponent(
    componentName: string,
    importStatement?: ImportStatement,
  ) {
  ...
  function appendToParent<T extends Mdast.Parent, C extends Mdast.RootContent>(
    parentNode: T,
    node: C,
  ): C | Mdast.RootContent {
    if (unistRoot === null) {
      unistRoot = node as unknown as Mdast.RootContent;
      return unistRoot;
    }
  ...
  function visit(
    ...
    usedVisitors: Set<number> | null = null,
  ) {
      ...
      actions: {
        ...
        nextVisitor() {
          visit(
            lexicalNode,
            mdastParent,
            (usedVisitors ?? new Set()).add(visitors.indexOf(visitor)),
          );
          return;
        },
      },
  ...
  for (const componentName of referredComponents) {
    const descriptor =
      jsxComponentDescriptors.find(
        (descriptor) => descriptor.name === componentName,
      ) ??
  ...
  // if (frontmatter) {
  //   typedRoot.children.splice(
  //     typedRoot.children.indexOf(frontmatter) + 1,
  //     0,
  //     ...imports,
  //   );
  // } else {
      ...
      if (onlyChild.type === 'mdxJsxTextElement' && onlyChild.name === 'span') {
        onlyChild.attributes.forEach((attribute) => {
          if (attribute.type === 'mdxJsxAttribute') {
            const parentAttribute = node.attributes.find(
              (attr) =>
                attr.type === 'mdxJsxAttribute' && attr.name === attribute.name,
            );
            if (parentAttribute) {
...
function fixWrappingWhitespace(
  node: Mdast.Parent | Mdast.RootContent,
  parentChain: Mdast.Parent[],
) {
  if (node.type === 'strong' || node.type === 'emphasis') {
    const lastChild = node.children.at(-1);
    if (lastChild?.type === 'text') {
      const trailingWhitespace = lastChild.value.match(
      ...
      if (trailingWhitespace) {
        ...
        if (parent) {
          parent.children.splice(
            parent.children.indexOf(node as unknown as Mdast.RootContent) + 1,
            0,
            {
              type: 'text',
    ...
    if (firstChild?.type === 'text') {
      const leadingWhitespace = firstChild.value.match(
      ...
      if (leadingWhitespace) {
        ...
        if (parent) {
          parent.children.splice(
            parent.children.indexOf(node as unknown as Mdast.RootContent),
            0,
            {
              type: 'text',
...
type ToMarkdownExtension = NonNullable<ToMarkdownOptions['extensions']>[number];

/**
 * @internal
 */
interface ExportMarkdownFromLexicalOptions extends ExportLexicalTreeOptions {
  visitors: LexicalVisitor[];
