...
import { RC5LinkDialog } from '../link/RC5LinkDialog';

/**
 * Link Dialog cloned from MdxEditor in order to fix a bug when updating display text
 */
            ...
            () => {
              ...
              if (!linkNode) {
                ...
                if (firstChild && $isTextNode(firstChild)) {
                  firstChild.setTextContent(linkContent);
                } else {
                  linkNode.clear();
                  linkNode.append($createTextNode(linkContent));
                }
              }
            },
            { discrete: true },
...
export type ReadOnlyClickLinkCallback = (
  event: MouseEvent,
  node: LinkNode,
  url: string,
) => void;
...
/** @internal */
export const onReadOnlyClickLinkCallback$ =
  Cell<ReadOnlyClickLinkCallback | null>(null, (r) => {
    r.pub(createActiveEditorSubscription$, (editor) => {
      function onClick(event: MouseEvent) {
        const [readOnly, callback] = r.getValues([
          readOnly$,
...
export const linkDialogPlugin = realmPlugin<{
  ...
  init(r, params) {
    r.pub(addComposerChild$, params?.LinkDialog ?? RC5LinkDialog);
    r.pub(onClickLinkCallback$, params?.onClickLinkCallback ?? null);
    r.pub(
      onReadOnlyClickLinkCallback$,
      params?.onReadOnlyClickLinkCallback ?? null,
    );
  },
  update(r, params = {}) {
    r.pub(
      linkAutocompleteSuggestions$,
      params.linkAutocompleteSuggestions ?? [],
    );
  },
});
