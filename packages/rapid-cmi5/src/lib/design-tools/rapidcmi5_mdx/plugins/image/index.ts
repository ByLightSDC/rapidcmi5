...
const internalInsertImage$ = Signal<SrcImageParameters>((r) => {
  r.sub(
    r.pipe(internalInsertImage$, withLatestFrom(activeEditor$)),
    ([values, theEditor]) => {
      theEditor?.update(() => {
...
export const insertImage$ = Signal<InsertImageParameters>((r) => {
  r.sub(
    r.pipe(insertImage$, withLatestFrom(imageUploadHandler$)),
    ([values, imageUploadHandler]) => {
    ...
    return mergeRegister(
      ...
      editor.registerCommand(
        ...
        (event: ClipboardEvent) => {
          ...
          const isMixedPayload = cbPayload.some(
...
export const openNewImageDialog$ = Action((r) => {
  r.link(
    r.pipe(openNewImageDialog$, mapTo({ type: 'new' })),
    imageDialogState$,
  );
});
 ...
 * @group Image
 */
export const openEditImageDialog$ = Signal<
  Omit<EditingImageDialogState, 'type'>
>((r) => {
  r.link(
    r.pipe(
      openEditImageDialog$,
...
export const closeImageDialog$ = Action((r) => {
  r.link(
    r.pipe(closeImageDialog$, mapTo({ type: 'inactive' })),
    imageDialogState$,
  );
});
 ...
 * @group Image
 */
export const editImageToolbarComponent$ =
  Cell<React.FC<EditImageToolbarProps>>(EditImageToolbar);
...
export const imagePlugin = realmPlugin<{
  ...
  init(realm, params) {
    realm.pubIn({
      ...
      [disableImageSettingsButton$]: Boolean(
        params?.disableImageSettingsButton,
      ),
